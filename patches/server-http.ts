...
type HookDispatchers = {
  ...
  dispatchAgentHookStreaming?: (value: HookAgentDispatchPayload, res: ServerResponse) => Promise<void>;
};
...
export function createHooksRequestHandler(
  ...
  return async (req, res) => {
    ...
    if (subPath === "agent") {
      ...
      // Check for streaming parameter (SSE support for Paperclip)
      const streamParam = url.searchParams.get("stream");
      const wantsSse = streamParam === "true" || streamParam === "sse" || 
                       (req.headers.accept || "").includes("text/event-stream");
      
      if (wantsSse && dispatchAgentHookStreaming) {
        // SSE streaming mode - run agent and stream response
        await dispatchAgentHookStreaming({
          ...normalized.value,
      ...
      // Non-streaming mode - return runId immediately
